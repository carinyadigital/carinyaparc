...
**Related:** [ADR-0001](ADR-0001-astro-mdx-replaces-payload.md) (why the CMS was removed), [`docs/architecture/astro-migration.md`](../architecture/astro-migration.md) §8 (editorial workflow), [`docs/architecture/solution.md`](../architecture/solution.md) §5.2 (publish runtime view)
...
- **The authoring contract for agents.** An agent writing content writes MDX or YAML under `content/` only, following the frontmatter contract in `astro-migration.md` §3 and the schemas in `apps/web/src/content.config.ts`; sets `draft: true` unless the brief says otherwise; never edits `apps/`, `packages/` or `.github/` in a content pull request; and leaves the merge to a human. Content review (`content-seo-review`) runs on the pull request.
