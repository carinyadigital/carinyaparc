...
```bash
pnpm test                                        # Vitest unit tests for apps/web via Turbo
...
- CSP is a host allowlist with `'unsafe-inline'` for scripts, because static HTML cannot carry per-request nonces. It is enforced as `Content-Security-Policy`; violations POST to `/api/csp-report/`. Do not add hosts or loosen directives without explicit approval.
...
- Rate limiting is in-memory per serverless instance. It is a nuisance filter, not a guarantee; durable abuse control is a Vercel WAF rule on `POST` to `/api/contact`, `/api/subscribe` and `/api/events`.
